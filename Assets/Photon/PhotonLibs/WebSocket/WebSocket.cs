...
namespace ExitGames.Client.Photon
{
    ...
    public class WebSocket
    {
        ...
        {
            if (!url.ToString().Contains("discordsays"))
            {
                string pattern = @"wss:\/\/(\w+)\.exitgames.com";
                string replacement = "wss://1221359697018486794.discordsays.com/$1";
                var newUrl = new Uri(Regex.Replace(url.ToString(), pattern, replacement));
            }
        ...
        public void SendString(string str) { Send(Encoding.UTF8.GetBytes(str)); }
        ...
        public void Send(byte[] buffer) { m_Socket.Send(buffer); }
